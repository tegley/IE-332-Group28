...
$whereStateShip = "WHERE s.ActualDate BETWEEN '" . $tmp[0] . "' AND '" . $tmp[1] . "'"; //User specified time range //If user specifies date range
    ...
    $shipmentsOutstandingSelect = "SELECT s.ShipmentID, d.CompanyID, c.CompanyName, (CASE WHEN s.ActualDate IS NULL THEN 1 ELSE 0 END) AS CurrentlyOut
    FROM Distributor d JOIN Company c ON d.CompanyID = c.CompanyID JOIN Shipping s ON s.DistributorID = d.CompanyID";
    $shipmentsOutstandingQuery = "{$shipmentsOutstandingSelect} {$whereStateShip} AND c.CompanyName =  '" . $quer[0] . "' {$groupState};"; //Will have same filters as user specified
    ...
    $disruptionEventQuery = "{$disruptionEventSelect} {$whereStateShip} AND s.ActualDate BETWEEN e.EventDate AND e.EventRecoveryDate AND c.CompanyName =  '" . $quer[0] . "'; "; //Will have same time range filters as user specified
