...
$groupState = "";
    ...
    if (!empty($quer[1])){
    $whereDis = " WHERE c.CompanyName =  '" . $quer[1] . "'";

    $distributorSELECT = "SELECT d.CompanyID AS DistributorID, c.CompanyName, COUNT(DISTINCT s.ShipmentID) AS ShipmentVolume, ROUND(((SUM(CASE WHEN s.ActualDate <= s.PromisedDate THEN 1 ELSE 0 END) / COUNT(DISTINCT s.ShipmentID)) * 100), 2) AS OTRate, p.ProductName, p.ProductID
    ...
    $shipmentsOutstandingQuery = "{$shipmentsOutstandingSelect} {$whereStateShip} AND c.CompanyName =  '" . $quer[1] . "' {$groupState};"; //Will have same filters as user specified
    ...
    $disruptionEventQuery = "{$disruptionEventSelect} {$whereStateShip} AND s.ActualDate BETWEEN e.EventDate AND e.EventRecoveryDate AND c.CompanyName =  '" . $quer[1] . "'; "; //Will have same time range filters as user specified
    ...
    JOIN ImpactsCompany i ON i.AffectedCompanyID = d.CompanyID JOIN DisruptionEvent e ON e.EventID = i.EventID JOIN DisruptionCategory y ON y.CategoryID = e.CategoryID";
    $disruptionHIGHEventQuery = "{$disruptionHIGHEventSelect} {$whereStateShip} AND s.ActualDate BETWEEN e.EventDate AND e.EventRecoveryDate AND i.ImpactLevel = 'High' AND c.CompanyName =  '" . $quer[1] . "'; "; //Will have same time range filters as user specified
