        ...
        <div class="col-md-9">
            ...
            <div id="companySection" class="collapse-content">
                ...
                <form name="myForm">
                    ...
                    <select id="myForm" class="form-select mb-3">
                    ...
                    <input type="text" id="company_input" class="form-control mb-2" placeholder="Enter Company Name">
                    </div>

                    <!-- City sub-filter -->
                    <div id="cityFilter" style="display:none;"> 
                    <input type="text" id="city_input" class="form-control mb-2" placeholder="Enter City">
                    </div>
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-6 text-center">
                        <div class="stats-header">Quanitiy Shipped versus Received over time</div>
    ...
    xhttp.onload = function() {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            try {
                ...
                // Calculate net difference (shipped - received)
                // Aggregate shipments by date
                const shipmentsByDate = {};
                data.shipping.forEach(item => {
                    const date = item.ActualDate;
                    shipmentsByDate[date] = (shipmentsByDate[date] || 0) + parseInt(item.Quantity);
                });

                // Aggregate receivings by date
                const receivingsByDate = {};
                data.receivings.forEach(item => {
                    const date = item.ReceivedDate;
                    receivingsByDate[date] = (receivingsByDate[date] || 0) + parseInt(item.QuantityReceived);
                });

                // Get all unique dates and sort them
                const allDates = [...new Set([...Object.keys(shipmentsByDate), ...Object.keys(receivingsByDate)])].sort();

                // Calculate net difference for each date
                const netDifference = allDates.map(date => {
                    const shipped = shipmentsByDate[date] || 0;
                    const received = receivingsByDate[date] || 0;
                ...
                var trace = {
                    ...
                    y: netDifference,
                    type: 'bar',
                    name: 'Net Flow (Shipped - Received)',
                    marker: {
                        color: netDifference.map(val => val >= 0 ? '#0f6fab' : '#dc3545') //blue will mean more shipped than received (positive values, red means more recieved tahn shipped, negative values)
                    }
                };
                ...
                var layout = {
                    title: 'Net Product Flow Over Time (Shipped - Received)',
                    xaxis: { title: 'Date' },
                ...
                Plotly.newPlot('lineChartAdjustments', plotData, layout, {responsive: true});
