<?php
$servername="mydb.itap.purdue.edu";

$username = "g1151938";//yourCAREER/groupusername
$password = "Purdue28";//yourgrouppassword
$database=$username;//ITaPsetupdatabasename=yourcareerlogin

$conn=new mysqli($servername,$username,$password,$database);
if($conn->connect_error){
die("Connection failed:" . $conn->connect_error);
}

//Temporary arrays
$tmp= array($user_username,$user_password); //[0]=Username,[1]=Password
$result_array = array(); //Username is 0; Password is 1

//Query - check if username is in the database
$check_username = "SELECT Username, Password FROM User WHERE BINARY Username='" .$tmp[0]. "'";
$result_check_username = mysqli_query($conn, $check_username);
$first_result = mysqli_fetch_array($result_check_username, MYSQLI_NUM); //Capture query result
if ($first_result[0] == NULL) { //Check is username is valid
    $result_array[0] = "0"; //Empty result
    $result_array[1] = "Username is incorrect"; //Invalid
    $conn->close();
}
else {
    //Query - check if password is correct BINARY allows case sensitivity checks
    $check_password_correct = "SELECT Username, Password FROM User WHERE BINARY Username='" .$tmp[0]. "' AND BINARY Password='" .$tmp[1]. "'";
    $result_check_password_correct = mysqli_query($conn, $check_password_correct);
    $second_result = mysqli_fetch_array($result_check_password_correct,MYSQLI_NUM); //Capture query result
    if($second_result[0]==NULL) { //Check if password is correct
        $result_array[0] = $tmp[0]; //Username
        $result_array[1] = "Password is incorrect"; //Invalid
        $conn->close();
        }
    else {
        //Query - return role from database
        $sql = "SELECT Role, FullName FROM User WHERE BINARY Username='" .$tmp[0]. "'AND BINARY Password='" .$tmp[1]. "'";
        $final_result=mysqli_query($conn, $sql);
        $parsed_result = mysqli_fetch_array($final_result, MYSQLI_NUM);
        $result_array[0] = $parsed_result[0]; //Role
        $result_array[1] = $parsed_result[1]; //Full Name
        $conn->close();
    }
}
?>

